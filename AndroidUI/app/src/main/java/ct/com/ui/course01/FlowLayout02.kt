 ...
 * 需求2 加入子控件的外边距属性
 * 需求3 实现Gravity属性
 *TODO 实现Item的Gravity 属性 思路 在换行的时候 就可以确定行的最大高度
 */
class FlowLayout02 @JvmOverloads constructor(
    context: Context?,
    ...
    private val layoutGravity = Gravity.CENTER_VERTICAL
    ...
    //定义一个集合 保存每个Item的原点
    private val mPointList = mutableListOf<List<Rect>>()


    //记录一行的最大宽度 用于横向居中
    private var transItemMaxWidth = 0

    //记录每行的高度 用于垂直居中
    private val mLongList = mutableListOf<Int>()
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        mPointList.clear()
        mLongList.clear()
        var transList = mutableListOf<Rect>()
...
//            )
            rect.left = tempWidth + paddingLeft
            rect.top = realHeight
            if (widthOffer < tempWidth + childWidth) {
                transItemMaxWidth = max(transItemMaxWidth, tempWidth)
                ...
                mLongList.add(realHeight - intervalHeight)
                mPointList.add(transList)
                transList = mutableListOf()
            }
        ...
        mLongList.add(realHeight - paddingBottom)
        mPointList.add(transList)

        //根据Gravity变换每个Rect
        transformRectByGravity(layoutGravity)
        //测量 主要是针对 AT_MOST 模式
...
//            realHeight = heightOffer
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        var index = 0
        for (tranList in mPointList) {

            for (rect in tranList) {
                val childView = getChildAt(index)
                index++
                childView.layout(rect.left, rect.top, rect.right, rect.bottom)
                Log.e("TAG", "布局:$index -- $rect")
            }
...
//    }
     ...
     * 根据 Gravity 变换坐标
     * */
    private fun transformRectByGravity(gravity: Int) {

        when (gravity) {
            Gravity.CENTER_HORIZONTAL -> transformHorizontal()
            Gravity.CENTER_VERTICAL -> transformVertical()
            Gravity.CENTER -> {
                transformHorizontal()
                transformVertical()
            }
    ...
    private fun transformHorizontal() {
        var maxWidth = 0
        for (tranList in mPointList) {
            maxWidth = max(tranList[tranList.size - 1].right, maxWidth)
        }

        for (tranList in mPointList) {
            if (tranList[tranList.size - 1].right >= maxWidth)
                continue
            val offset = (maxWidth - tranList[tranList.size - 1].right) / 2
            for (rect in tranList) {
                rect.left += offset
                rect.right += offset
            }
    ...
    private fun transformVertical() {
        var lastHeight = paddingTop
        Log.e("TAG", "高度控制器:${mLongList} --- ${mPointList}")
        for (index in mLongList.indices) {
            val transList = mPointList[index]
            val maxHeight = mLongList[index] - lastHeight

            for (rect in transList) {
                val height = rect.height()
                Log.e("TAG", "本行最高高度:$maxHeight --- $height")
                if (height >= maxHeight) {
                    lastHeight = rect.bottom + intervalHeight
                    continue
                }
                val offset = (maxHeight - height) / 2
                rect.top += offset
                rect.bottom = rect.top + height

            }

            //lastHeight = mLongList[index] + intervalHeight
        }
    }
