...
class FishDrawable : Drawable() {
    ...
    //鱼起始角度
    private val fishMainAngle = 40.0
    ...
    override fun draw(canvas: Canvas) {
        ...
        //获取 鱼鳍起始点的坐标
        //右鱼鳍起始点坐标
        val rightFinsStartPoint = calculatePoint(headPoint, FINS_START_LENGTH, -110.0)
        drawFins(canvas, rightFinsStartPoint, fishAngle, true)

        //左鱼鳍起始点坐标
        val leftFinsStartPoint = calculatePoint(headPoint, FINS_START_LENGTH, 110.0)
        ...
        //获取鱼尾第二个圆的圆心
        val middlePoint =
            calculatePoint(bodyBottomCenterPoint, FISHTAIL_BIG_LENGTH, fishAngle - 180)

        //获取鱼尾底部圆的圆心
        val smallPoint = calculatePoint(middlePoint, FISHTAIL_SMALL_LENGTH, fishAngle - 180)
        ...
        //绘制三角形
        drawTriangle(canvas, middlePoint, BIG_TRIANGLE_LENGTH, MIDDLE_RADIUS, fishAngle)
        drawTriangle(canvas, middlePoint, SMALL_TRIANGLE_LENGTH, MIDDLE_RADIUS * 0.75f, fishAngle)
    ...
    private fun drawBody(
        ...
        val controlLeftPoint = calculatePoint(topLeftPoint, BODY_LENGTH * 0.5f, fishAngle + quadAngle)
    ...
    private fun drawFins(
        ...
        //获取控制点的坐标
        val controlAngle = if (isDrawRight) fishAngle - 115f else fishAngle + 115f
     ...
     *
     * */
    private fun calculatePoint(startPointF: PointF, length: Float, angle: Double): PointF {
